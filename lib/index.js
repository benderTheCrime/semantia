...
class Semantia {
    static testCase() {
        ...
        if (!obj.hasOwnProperty('skip')) {
            ...
            this.skipTestCase.apply(this, arguments);
        }
    ...
    static skipTestCase() {
        const [ fn, obj ] = this._handleTestCaseArguments.apply(this, arguments);
        this.skipped = (this.skipped || 0) + (obj.callCount || 1);

        return this;
    }

    static skipTestCases(fn, ...args) {
        for (let arg of args) {
            this.skipTestCase(fn, arg);
        }
...
function generateNumericArgument({ order, max = 1, min = 0 }) {
    let rand = Math.random(),
        rnd; // = Math.random() * order + 1;
